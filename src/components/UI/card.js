...
export default function card({image, name, location, description, github, frontendMentor, linkdin, x, insta}) {
  return (
    <div className='flex items-center flex-col bg-dark-grey p-8 rounded-lg text-white w-500'>
        <img src={image} alt="profile" className="rounded-full w-24 h-24 mb-6"/>
        <h1 className='text-2xl font-meduim mb-0.5'>{name}</h1>
        <h3 className='text-primary text-sm mb-4'>{location}</h3>
        <p className='font-normal text-sm  mb-5 text-gray-300'>" {description} "</p>
        <Links  
        github={github} 
